...
sub FindGhostExons
{
	...
	for (my $s2=$s1+1; $s2<$num_species; $s2++) {
		...
		#  UPDATE:  We're going to just pull micro exons into any adjacent
		#           "nasty" exons, even if that makes it possible that
		#           they might be pulled into multiple searches.
		#
		...
		#
		# UPDATE (2024, Jan 6th <- the fateful J6!)
		#   We're actually NOT going to concatenate contiguous nasty exons,
		#   since (1) this allows us to better catch rearrangements and
		#   (2, much more importantly) avoids homologous overextension.
		#
		my @NastyRunStarts;
		my @NastyRunEnds;
		my $num_nasty_runs = 0;
		for (my $exon_id=0; $exon_id<$num_exons; $exon_id++) {
			if ($IsNastyExonPair[$exon_id]) {
				...
				$NastyRunEnds[$num_nasty_runs]   = $exon_id;
			
				# Pull in any adjacent micro exons
				for (my $walk_left = $exon_id-1; $walk_left >= 0; $walk_left--) {
					if ($MicroExonic[$walk_left]) {
						$NastyRunStarts[$num_nasty_runs] = $walk_left;
					} else {
						last;
					}
				}
				for (my $walk_right = $exon_id+1; $walk_right < $num_exons; $walk_right++) {
					if ($MicroExonic[$walk_right]) {
						$NastyRunEnds[$num_nasty_runs] = $walk_right;
					} else {
						last;
					}
				}
